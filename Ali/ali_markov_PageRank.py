        ...
        for s in range(len(tm)):
            ...
            for e in range(len(tm[0])):
                if s != e and tm[s, e] != 0:
                    ...
                    label = (
                        Text(str(matrix_prob), font="Consolas")
...
class MarkovChainSimulator:
    ...
    def init_users(self):
        ...
        self.users = [
            ...
            .set_color(YELLOW)
            .set_opacity(0.6)
            .set_stroke(YELLOW, width=2, opacity=0.8)
...
class MarkovChainIntroEdited(Scene):
    def construct(self):
        ...
        new_labels = [self.get_label(markov_chain_g.edges[edge], prob).set_color(YELLOW) for edge, prob in new_edges_to_labels]
    ...
    def discuss_markov_prop(self, markov_chain_g):
        ...
        for edge in markov_chain_g.edges:
            if edge == (3, 2) or edge == (3, 1):
                highlight_animations.extend(
                    [markov_chain_g.labels[edge].animate.set_color(YELLOW)]
                )
            else:
                highlight_animations.extend(
                    [
...
class StationaryDistPreview(Scene):
    def construct(self):
        ...
        point_3 = Text("3. How do we efficiently compute them?", font="CMU Serif").scale(0.5)
...
class Uniqueness(Scene):
    def construct(self):
        ...
        for user_id in state_to_users[1]:
            users[user_id].set_stroke(color=GREEN_E).set_fill(
                color=GREEN_E, opacity=0.8
            )
...
class ChallengeSeparated(Scene):
    def construct(self):
        ...
        challenge[1][-16:-1].set_color(YELLOW)
...
class Periodicity(Scene):
    ...
    def get_distribution_plot(self, markov_chain, num_steps, axes_position=RIGHT*3, ax_width=5, ax_height=5, dist=None):
        ...
        custom_y_label = Text("1.0", font="Consolas").scale(0.4)
        ...
        custom_x_label = Text(f"{num_steps}", font="Consolas").scale(0.4)
        custom_x_label_pos = axes.x_axis.n2p(num_steps) + DOWN * MED_SMALL_BUFF
        custom_x_label.move_to(custom_x_label_pos)

        x_axis_label = Text("Step/Iteration", font="Consolas").scale(0.5).next_to(axes.x_axis, DOWN)
...
class IntroduceBigTheoremText(Scene):
    def construct(self):
        ...
        how_to_calculate_it = Text("How to calculate the stationary distribution?", font="CMU Serif", weight=BOLD).scale(0.7).shift(DOWN * 3)
...
class PageRankExtraText(Scene):
    def construct(self):
        # problem = Text("How to rank states 0, 1 and 2?", font="CMU Serif").scale(0.8).move_to(DOWN * 3.3)
        ...
        problem_2 = Text("How to rank states 2 and 3?", font="CMU Serif").scale(0.8).move_to(DOWN * 3.3)
...
class PageRank(IntroduceBigTheorem1):
    ...
    def show_pagerank_cxn(self):
        ...
        page_rank = Text("PageRank", font="CMU Serif", weight=BOLD)
        arrow = Tex(
            r"$\bm{\Updownarrow}$",
            tex_template=template
        ).scale(1.5)
        stationary_dist_text = Text("Stationary Distributions of Markov Chains", font="CMU Serif", weight=BOLD).scale(0.8)
        ...
        some_added_complexity = Text("(*) In practice, it's a bit more complicated", font="CMU Serif").scale(0.7)
    ...
    def intro_issue(self):
        ...
        problem = Text("How to rank states 2 and 3?", font="CMU Serif").scale(0.8).move_to(DOWN * 3.3)
...
class PageRankSolution(Scene):
    def construct(self):
        text = Text("Idea: randomly select a new state", font="CMU Serif").scale(0.8).move_to(UP * 3.5)
        self.play(
            Write(text),
            run_time=2
        )
...
class PageRankRecap(Scene):
    def construct(self):
        title = Text("PageRank Algorithm", font="CMU Serif", weight=BOLD)
        title.move_to(UP * 3.5)
...
class EigenValueMethodFixed2(Scene):
    ...
    def show_eigen_concept(self, transpose_transition_eq):
        ...
        title = Text("Eigenvalues/Eigenvectors", font="CMU Serif", weight=BOLD).scale(0.8)
        title.move_to(UP * 3.5)
        ...
        left_plane = NumberPlane(
            ...
            # faded_line_ratio=4,
            axis_config={"stroke_color": PURPLE},
        ).move_to(LEFT * 3.5 + DOWN * 1)

        left_surround_plane = SurroundingRectangle(left_plane, buff=0, color=PURPLE)
        ...
        right_surround_plane = SurroundingRectangle(right_plane, buff=0, color=PURPLE)
    ...
    def matrix_to_mob(self, matrix: np.ndarray, has_background_color=False):
        ...
        return Matrix(
            ...
            element_to_mobject_config={"font": "Consolas"},
            h_buff=2.3,
            v_buff=1.3,
        )
...
class BeforeAfterComparison(Scene):
    def construct(self):
        ...
        bar_title = Text("Search Engine Usage in 1997", font="CMU Serif", weight=BOLD).scale(0.7)
        y_axis_label = Text("Usage (%)", font="CMU Serif").scale(0.6)

        chart = BarChart(
            values=bar_values,
            bar_names=bar_names,
            bar_colors=[YELLOW, PURPLE, GREEN_E, PURPLE, GREEN_D],
            y_range=[0, 100, 25],
            y_length=1.8,
            x_length=8,
        ...
        bottom_title = Text("Search Engine Usage in 2022", font="CMU Serif", weight=BOLD).scale(0.7)
