...
class TransitionMatrixCorrected3(MovingCameraScene):
    def construct(self):
        ...
        # math_notation_title = (
        #     Text("Some bits of mathematical notation",  weight=BOLD)
        #     .scale(0.5)
        ...
        surr_rect = SurroundingRectangle(
            trans_matrix_mob[0][0 : len(markov_ch.get_states())], color=YELLOW
        )
        ...
        stationary_dist_annotation = (
            Text("A distribution is stationary if:",  weight=BOLD)
            .scale(0.65)
    ...
    def get_current_count_mobs(self, markov_chain_g, markov_chain_sim, use_dist=False):
        ...
        for v in vertex_mobs_map:
            if not use_dist:
                ...
                label = Text(str(state_counts[v])).scale(0.6)
            else:
                state_counts = markov_chain_sim.get_user_dist(round_val=True)
                label = Text("{0:.2f}".format(state_counts[v])).scale(0.6)
...
class EigenvalueMethod(MovingCameraScene):
    def construct(self):
        ...
        purple_plane = NumberPlane(
            ...
            # faded_line_ratio=4,
            axis_config={"stroke_color": PURPLE, "stroke_width": 0},
        )
        ...
        static_plane = NumberPlane(
            ...
            axis_config={"stroke_color": PURPLE, "stroke_opacity": 0},
        ).set_opacity(0.6)
        ...
        distorted_vector = Vector([1, 0]).set_color(YELLOW)
        ...
        self.wait()
        eig_vector = Vector(direction=e_vecs[0]).set_color(YELLOW)
        self.play(
...
class PerformanceEvaluationFull(IntroWebGraph, SystemOfEquationsMethod, Periodicity):
    ...
    def intro_pip(self):
        title = (
            Text("Which method will perform best?",  weight=BOLD)
            .scale(0.8)
            .to_edge(UP)
        )
...
class PerformanceText(Scene):
    def construct(self):
        ...
        methods = VGroup(
            Text("Brute Force").scale(scale),
            Text("System of Equations").scale(scale),
            Text("Eigenvalues/Eigenvectors").scale(scale)
        )
